...
// Body: { prompt, references?: [{ data: base64, mimeType, note? }] }
// Generated bytes are persisted to the workspace folder under
// generated-images/ and referenced by path, so message history stays light
// and the gallery can be cached client-side.
groupRoutes.post('/:jid/generate-image', authMiddleware, async (c) => {
  ...
  for (const ref of parsed.data.references ?? []) {
    references.push({
      data: new Uint8Array(Buffer.from(ref.data, 'base64')),
  ...
  try {
    const image = await generateWorkspaceImage(
      buildReferenceAwarePrompt(parsed.data.prompt, references),
      imageConfig.model,
      references.map(({ data, mimeType }) => ({ data, mimeType })),
    );

    // Persist the generated bytes as a workspace file; the message attachment
    // references it by relative path instead of embedding base64.
    const ext =
      image.mimeType === 'image/jpeg' ? 'jpg' : image.mimeType.split('/')[1];
    const timestamp = new Date().toISOString();
    const fileName = `generated-${timestamp.replace(/[:.]/g, '-')}-${crypto
      .randomUUID()
      .slice(0, 8)}.${ext}`;
    const relativeDir = 'generated-images';
    const absoluteDir = path.join(getFileRoot(group.folder), relativeDir);
    ...
    const referenceSummary =
      references.length > 0
        ? `（参考 ${references.length} 张图片${
            references.some((r) => r.note) ? '，含分图说明' : ''
          }）`
        : '';
    const requestContent = `生成图片${referenceSummary}：${parsed.data.prompt}`;
    const attachments = JSON.stringify([
      { type: 'image', path: relativePath, mimeType: image.mimeType },
    ]);
    ...
    return c.json({
      ...
      image: { path: relativePath, mimeType: image.mimeType },
    });
  } catch (err) {
...
/**
 * Compose the final prompt sent to the image model: the user prompt plus a
 * numbered description of what each reference image should contribute. This
 * is how "multiple references with separate instructions" is expressed —
 * the upstream edits endpoint takes one shared prompt for all images.
 */
function buildReferenceAwarePrompt(
  ...
  const notes = references
    .map((ref, i) => (ref.note ? `参考图 ${i + 1}：${ref.note}` : null))
...
// Returns persisted generation rows with path-based attachments only; much
// cheaper than the full message page the studio page used to parse.
groupRoutes.get('/:jid/generated-images', authMiddleware, async (c) => {
  const jid = c.req.param('jid');
  const group = getRegisteredGroup(jid);
  if (!group) return c.json({ error: 'Group not found' }, 404);
  ...
  const limit = Math.min(
    Number.isFinite(limitRaw) ? Math.max(1, limitRaw) : 100,
    200,
  );

  const rows = getGeneratedImageMessages(jid, limit);
  const images = rows.flatMap((row) => {
    ...
    for (const att of attachments) {
      if (
        typeof att !== 'object' ||
        att === null ||
        (att as { type?: unknown }).type !== 'image'
      ) {
        continue;
      }
      const {
      ...
      // Legacy rows embed base64 directly; keep them renderable.
      if (imagePath) {
        out.push({
          messageId: row.id,
